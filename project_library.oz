...
fun {Times Rec I Val}
    if I == 0 then 
        Rec
    else {Times {AdjoinAt Rec I {Val}} I-1 Val}
    end
end
...
in
    if Start == End then 
        Res
    else {Repeat Start+1 End Res Fun}
    end
end
...
fun {InitCells Spaceship I N} % MAY BE UNNECESSARY
    ...
    in
        ((Spaceship.positions.1.x - Direction.1*I)#(Spaceship.positions.1.y - Direction.2*I)) | {InitCells Spaceship I+1 N} 
    end
end
    ...
    fun {TailBombs Spaceships BombList}
        case Spaceships of Spaceship|Rest then
            if {IsAlive Spaceship} andthen Spaceship.seismicCharge.1 then
                BombList1 = bomb(explodesIn:Config.bombLatency position:{List.last Spaceship.positions}) | BombList
            in
                {TailBombs Rest BombList1}
            else {TailBombs Rest BombList}
            end
        [] nil then BombList 
        end
    end
...
in
    {Adjoin 
        Config
        config(
            bombs: {TailBombs Config.spaceships Config.bombs} % {EffectBombs Config.spaceships nil {TailBombs Config.spaceships Config.bombs}}
            spaceships: {Map Config.spaceships
                fun{$ S}
                    {Adjoin S spaceship(seismicCharge:S.seismicCharge.2)} % effects:{Filter S.effects fun{$ E} {Label E} \= bomb end}
                end
            }
        )
    }
...
fun {UpdateBombs Config}
    fun {UpdateBombs Bombs}
        case Bombs of Bomb|Rest then
            if Bomb.explodesIn == 0 then
                {UpdateBombs Rest}
            else {AdjoinAt Bomb explodesIn Bomb.explodesIn-1} | {UpdateBombs Rest}
            end
        [] nil then 
            nil 
        end
...
in
    {AdjoinAt Config bombs {UpdateBombs Config.bombs}}
end
...
fun {IsAlive Spaceship} 
    {Member death Spaceship.effects} == false 
end

fun {IsDead Spaceship} 
    {Member death Spaceship.effects} 
end
...
proc {Play ConfigName StudentNext StudentDecodeStrategy Options FinalResult}
    ...
    fun {DecodeStrategy Strategy}
        ...
        else {StudentDecodeStrategy Strategy} 
        end
    end
    ...
    in
        ...
        % TODO colin check puis remove
        case {Length WinningSpaceships} of 1 then 
            win(WinningSpaceships.1.team)
        else % If no immediate winner based on length, proceed with existing logic
            case {Length AliveTeams} of 1 then 
                win(AliveTeams.1)
            else
                if MovingSpaceships \= nil orelse Config.bombs \= nil then 
                    inprogress
                else tie(AliveTeams) 
                end
            end
    ...
    if Options.debug then 
                ...
                proc {ShowCells Cells IsHead IsAlive}
                    case Cells of Cell|Rest then
                        if IsHead then % TODO change
                            case Spaceship.positions.1.to of north then
                                ...
                                {C tk(create rectangle Cell.x*20-2 Cell.y*20+4 Cell.x*20-6 Cell.y*20+8 tags:@R fill:if IsAlive then red else black end)} % Left thruster
                                {C tk(create rectangle Cell.x*20+2 Cell.y*20+4 Cell.x*20+6 Cell.y*20+8 tags:@R fill:if IsAlive then red else black end)} % Right thruster
                            ...
                            [] south then
                                ...
                                % Thrusters (bottom, but symmetrically designed so orientation doesn't matter)
                                {C tk(create rectangle Cell.x*20-2 Cell.y*20-8 Cell.x*20-6 Cell.y*20-4 tags:@R fill:if IsAlive then red else black end)}
                                {C tk(create rectangle Cell.x*20+2 Cell.y*20-8 Cell.x*20+6 Cell.y*20-4 tags:@R fill:if IsAlive then red else black end)}
                            ...
                            [] east then
                                ...
                                {C tk(create rectangle Cell.x*20-8 Cell.y*20-2 Cell.x*20-4 Cell.y*20+6 tags:@R fill:if IsAlive then red else black end)} % Upper thruster
                                {C tk(create rectangle Cell.x*20-8 Cell.y*20+2 Cell.x*20-4 Cell.y*20-6 tags:@R fill:if IsAlive then red else black end)} % Lower thruster
                            ...
                            [] west then
                                ...
                                {C tk(create rectangle Cell.x*20+4 Cell.y*20-2 Cell.x*20+8 Cell.y*20+6 tags:@R fill:if IsAlive then red else black end)} % Upper thruster
                                {C tk(create rectangle Cell.x*20+4 Cell.y*20+2 Cell.x*20+8 Cell.y*20-6 tags:@R fill:if IsAlive then red else black end)} % Lower thruster
                            ...
                            {C tk(create rectangle Cell.x*20-8 Cell.y*20-8 Cell.x*20+8 Cell.y*20+8 tags:@R fill:Spaceship.team)}
                        in
                            % Using a lighter color to represent the content    
                            {C tk(create rectangle Cell.x*20-4 Cell.y*20-4 Cell.x*20+4 Cell.y*20+4 tags:@R fill:'lightgrey')}
                        end
        ...
        proc {ShowBonuses Bonuses}
            case Bonuses of Bonus|Rest then
                ...
                Y = Bonus.position.y
            in
                {C tk(create oval X*20-10 Y*20-10 X*20+10 Y*20+10 tags:@R fill:{BonusColor Bonus})}
                {ShowBonuses Rest}
            [] nil then skip
            end
        end
        ...
        proc {ShowBombs Bombs}
            ...
            in
                if Bomb.explodesIn == 0 then
                    {C tk(create rectangle (X-2)*20-6 Y*20-3 (X+2)*20+6 Y*20+3 tags:@R fill:red outline:red)}
                    {C tk(create rectangle X*20-3 (Y-2)*20-6 X*20+3 (Y+2)*20+6 tags:@R fill:red outline:red)}
                else
                    {C tk(create oval X*20-6 Y*20-6 X*20+6 Y*20+6 tags:@R fill:black)}
                    {C tk(create text X*20 Y*20 text: {IntToString Bomb.explodesIn} tags:@R fill:red)}
                end
                {ShowBombs Rest}
            [] nil then skip
            end
        ...
        proc {ShowDiff Diff}
            case Diff of (X#Y)|Rest then
                {C tk(create rectangle X*20-2 Y*20-8 X*20+2 Y*20+2 tags:@R fill:red)}
                {C tk(create rectangle X*20-2 Y*20+4 X*20+2 Y*20+8 tags:@R fill:red)}
                {ShowDiff Rest}
            [] nil then skip
            end
        end
        ...
        if @CurrentConfigs.1.walls then
            {C tk(create rectangle  0 0 W*20+20 H*20+20 tags:B fill:'blue' outline:black)}
            {C tk(create rectangle 30 30 W*20-10 H*20-10 tags:B fill:lightblue outline:black)}
        else
            {C tk(create rectangle 0 0 W*20+20 H*20+20 tags:B fill:yellow outline:black)}
            {C tk(create rectangle 10 10 W*20+10 H*20+10 tags:B fill:lightblue outline:black)}
        end
        {ShowConfig @CurrentConfigs}
            ...
            tkBind(
                ...
                action: proc{$ K}
                            ...
                            elseif K == space then
                                {NextConfig _}
                                {ShowConfig @CurrentConfigs}
                            else KeyDowns := {AdjoinAt @KeyDowns K {Boot_Time.getMonotonicTime}}
                            end
                        end
            )
        }
